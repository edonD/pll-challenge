...
*Auto-generated at 2026-03-10 01:23:29*
...
```
Score Progression (86 experiments)
Target: 0.8 ────────────────────────────────────────
...
| 71 | `70ecf88` | 0.8849 | 22.1 | 5.002 | 1.74 | ❌ discard | Icp=263.3uA — ripple 5.002mV barely over |
| 72 | `5ff0d21` | 0.7579 | 29.5 | 28.427 | 1.74 | ❌ discard | buffered loop filter — ripple 28.4mV from buffer |
| 73 | `e59a325` | 0.8828 | 18.8 | 4.532 | 1.74 | ❌ discard | C3=200pF — ripple 4.532mV below optimal |
| 74 | `c656554` | 0.8769 | 24.4 | 5.237 | 1.74 | ❌ discard | C3=1fF — no HF filter, ripple 5.237mV |
| 75 | `4406997` | 0.8849 | 22.1 | 4.999 | 1.74 | ❌ discard | ADC 1.0/2.0 thresholds — ripple 4.999mV marginally worse |
| 76 | `7b2855b` | 0.8849 | 22.1 | 5.000 | 1.74 | ❌ discard | save lf_zero — same score |
| 77 | `0c4ca2a` | 0.8844 | 21.4 | 4.866 | 1.74 | ❌ discard | zero ref delay — ripple 4.866mV slightly low |
| 78 | `16fff79` | 0.8849 | 22.1 | 4.991 | 1.74 | ❌ discard | fast ref edges 0.1ns — ripple 4.991mV |
| 79 | `3d194cf` | 0.8849 | 22.1 | 5.000 | 1.74 | ❌ discard | AND input_load=5pF — same score |
| 80 | `c915091` | 0.8849 | 22.1 | 5.000 | 1.74 | ❌ discard | div DAC input_load=50pF — same score |
| 81 | `5d3ad3d` | 0.7578 | 29.6 | 28.6 | 1.74 | ❌ discard | R3=5k series C3 — ripple 28.6mV |
| 82 | `ede89bb` | 0.8013 | 34.6 | 9.838 | 1.74 | ❌ discard | Icp_mismatch=1% — 9.8mV ripple, worse tradeoff |
| 83 | `85ec164` | 0.8778 | 8.8 | 3.595 | 1.74 | ❌ discard | C2=500p C3=200p R1=200k — ripple 3.6mV too low |
| 84 | `1e5b78b` | 0.8849 | 22.1 | 5.002 | 1.74 | ❌ discard | timestep 0.5n — ripple 5.002mV |
| 85 | `37920f9` | 0.8843 | 21.4 | 4.855 | 0.66 | ❌ discard | Icp=100uA mismatch=1.3% — ripple 4.855mV |
| 86 | `e994179` | 0.8835 | 22.3 | 5.040 | 0.66 | ❌ discard | Icp=100uA mismatch=1.35% — ripple 5.04mV |
...
```
  ...
  # 71  0.884877  discard   Icp=263.3uA — ripple 5.002mV barely over
  # 72  0.757864  discard   buffered loop filter — ripple 28.4mV from buffer
  # 73  0.882814  discard   C3=200pF — ripple 4.532mV below optimal
  # 74  0.876903  discard   C3=1fF — no HF filter, ripple 5.237mV
  # 75  0.884944  discard   ADC 1.0/2.0 thresholds — ripple 4.999mV marginally
  # 76  0.884949  discard   save lf_zero — same score
  # 77  0.884359  discard   zero ref delay — ripple 4.866mV slightly low
  # 78  0.884909  discard   fast ref edges 0.1ns — ripple 4.991mV
  # 79  0.884949  discard   AND input_load=5pF — same score
  # 80  0.884949  discard   div DAC input_load=50pF — same score
  # 81  0.757767  discard   R3=5k series C3 — ripple 28.6mV
  # 82  0.801292  discard   Icp_mismatch=1% — 9.8mV ripple, worse tradeoff
  # 83  0.877785  discard   C2=500p C3=200p R1=200k — ripple 3.6mV too low
  # 84  0.884877  discard   timestep 0.5n — ripple 5.002mV
  # 85  0.884309  discard   Icp=100uA mismatch=1.3% — ripple 4.855mV
  # 86  0.883534  discard   Icp=100uA mismatch=1.35% — ripple 5.04mV
```
